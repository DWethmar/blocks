...
export class GameScene extends Scene {
    ...
    constructor(private stage: PIXI.Container) {
        ...
        createCheckers(this.terrain, BlockType.GRASS, BlockType.VOID, [0, 0, 0]);
        createTower(this.terrain, BlockType.ROCK, [17, 15, 1]);
        createTower(this.terrain, BlockType.ROCK, [20, 18, 1]);
        createArch(this.terrain, BlockType.ROCK, [6, 1, 1]);

        for (let x = 0; x < 3; x++) {
            for (let y = 0; y < 3; y++) {
                createCheckers(this.terrain, BlockType.GRASS, BlockType.VOID, addPos([CHUNK_SIZE, -1, 0], [CHUNK_SIZE * x, CHUNK_SIZE * y, 0]));
                createTerrainNoise(this.terrain, BlockType.GRASS, BlockType.ROCK, addPos([CHUNK_SIZE, -1, 0], [CHUNK_SIZE * x, CHUNK_SIZE * y, 0]));
            }
        }

        this.terrain.addBlock([0, 0, 1], BlockType.ROCK);
        this.terrain.addBlock([1, 0, 1], BlockType.ROCK);
        this.terrain.addBlock([2, 0, 1], BlockType.ROCK);
        this.terrain.addBlock([3, 0, 1], BlockType.ROCK);
        this.terrain.addBlock([2, 0, 1], BlockType.ROCK);

        this.terrain.addBlock([8, 0, 1], BlockType.GRASS);
        this.terrain.addBlock([11, 0, 1], BlockType.GRASS);
        this.terrain.addBlock([CHUNK_SIZE, 0, 1], BlockType.VOID);
        ...
        bresenham3D(
            1, 0, 10,
            10, 0, 20
        ).forEach(p => this.terrain.addBlock(p, BlockType.SELECTION));
    ...
    selection() {
        ...
        const center: Point3D = [x, y, 0];

        const crossDiameter = 10;

        const left = minusPos(center, [crossDiameter, 0, 0]);
        const right = addPos(center, [crossDiameter, 0, 0]);

        const north = minusPos(center, [0, 0, crossDiameter]);
        const south = addPos(center, [0, 0, crossDiameter]);
