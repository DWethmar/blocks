...
export function getPointInCollection3D<T>(point: Point3D, collection: collection3d<T>): T {
...
export function setPointInCollection3D<T>(point: Point3D, collection: collection3d<T>, value: T): boolean {
    ...
    for (let x = 0; x < collection.length; x++) {
        for (let y = 0; y < collection[0].length; y++) {
            for (let z = 0; z < collection[0][0].length; z++) {
                const block = getPointInCollection3D(createPoint(x, y, z), collection);
                if (block) {
                    yield block;
                }
            }
